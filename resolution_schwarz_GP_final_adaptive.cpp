...
int main() {
    ...
    for(int N : N_values) {
        ...
        while (NormeGp > epsilon)
        {
            ...
            //------------------------------------------------------
            // 7) Adapter le pas de descente nu
            //------------------------------------------------------

            double tol = 1e-2 ;
            if (std::abs(currentCost - OldCost) < tol) {
            nu *= 1.1; 
            cout << "On augmente le pas " << currentCost << " " << OldCost << " " << nu <<endl ;
                if (N >= 40) {
                    if (nu > 3.){
                        nu = 0.5 * nu ;
                    }
                ...
                nu *= 0.8; 
                cout << "On diminue le pas " << currentCost << " "<< OldCost << " " << nu << endl ;
                    if (nu < 0.07){
                    nu = 1.2 * nu ;
                    }
            }
            ...
            //------------------------------------------------------
            // 8) Mise à jour de X^(n+1) = X^n - alpha * Gp
            //------------------------------------------------------
